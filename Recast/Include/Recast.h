...
/// 仅用于表示 tile 边界上一定范围的区域
/// 在构建 rcBuildRegions 时，将 chf.borderSize 指定的 tile 的边界区域按行或列给标记为不同的 regionId | RC_BORDER_REG
...
// 对于 reg_id，如果这个标记位被设置，代表 span 位于 tile 的边界区域（borderSize 指定的区域）
// 对于轮廓顶点，如果这个标记位被设置，代表可以在生成过程中被删掉而不影响轮廓线的效果
...
/// (Used during the region and contour build process.)
/// 代表该 span 是所属 region 最外面一圈的边缘 span
/// 只要该 span 有邻接 span 不属于同一个区域即可（包括不可行走区域）
/// @see rcCompactSpan::reg, #rcContour::verts, #rcContour::rverts
